...
use super::op::{ConditionCode, Location, Operand, Memory, Register, Op, REG_GPB2, REG_GPQ};
...
impl Encoder {
    ...
    /// as an escape code.
    fn emit_r_rm(&mut self, op_size: u8, mem: &Location, reg_num: u8, opcode: u32) {
        // Operand size override prefix.
        if op_size == 2 { self.emit_u8(0x66) }
        ...
        self.emit_rex(mem, reg_num, if op_size == 8 { 0x08 } else { 0 });
    ...
    fn emit_plusr(&mut self, op_size: u8, reg: Register, opcode: u32) {
    ...
    fn emit_alu(&mut self, dst: Location, src: Operand, id: u8) {
        ...
        match src.as_loc() {
            Err(imm) => {
                ...
                if op_size != 1 && i8::try_from(imm).is_ok() {
                ...
                if let Location::Reg(reg) = dst {
                    if reg as u8 & 0xF == 0 {
                        ...
                        self.emit_u8((id << 3) | if op_size == 1 { 0x04 } else { 0x05 });
                        self.emit_imm(if op_size == 8 { 4 } else { op_size }, imm);
                        return;
                    }
                }

                self.emit_r_rm(op_size, &dst, id, if op_size == 1 { 0x80 } else { 0x81 });
                self.emit_imm(if op_size == 8 { 4 } else { op_size }, imm);
            }
            ...
            Ok(src) => {
                ...
                match (dst, src) {
                    ...
                    (dst, Location::Reg(reg)) =>
                        self.emit_r_rm(op_size, &dst, reg as u8, (id << 3) as u32 | if op_size == 1 { 0x00 } else { 0x01 }),
                    (Location::Reg(reg), src) =>
                        self.emit_r_rm(op_size, &src, reg as u8, (id << 3) as u32 | if op_size == 1 { 0x02 } else { 0x03 }),
    ...
    fn emit_shift(&mut self, dst: Location, src: Operand, id: u8) {
        ...
        match src {
            ...
            Operand::Reg(Register::CL) =>
                self.emit_r_rm(op_size, &dst, id, if op_size == 1 { 0xD2 } else { 0xD3 }),
            Operand::Imm(1) =>
                self.emit_r_rm(op_size, &dst, id, if op_size == 1 { 0xD0 } else { 0xD1 }),
            Operand::Imm(imm) => {
                self.emit_r_rm(op_size, &dst, id, if op_size == 1 { 0xC0 } else { 0xC1 });
        ...
        match src.as_loc() {
            Err(imm) => {
                ...
                if let Location::Reg(reg) = dst {
                    ...
                    if op_size == 8 && u32::try_from(imm).is_ok() { op_size = 4 }

                    // If the above optimization is not possible, but imm is int32, then it is shorter to encode it using
                    // mod r/m.
                    if op_size != 8 || i32::try_from(imm).is_err() {
                        self.emit_plusr(op_size, reg, if op_size == 1 { 0xB0 } else { 0xB8 });
                ...
                check_imm_size(op_size, imm);
                self.emit_r_rm(op_size, &dst, 0, if op_size == 1 { 0xC6 } else { 0xC7 });
                self.emit_imm(if op_size == 8 { 4 } else { op_size }, imm);
            }
            ...
            Ok(src) => {
                ...
                match (dst, src) {
                    // Prefer INST r/m, r to INST r, r/m in case of INST r, r
                    (dst, Location::Reg(reg)) =>
                        self.emit_r_rm(op_size, &dst, reg as u8, if op_size == 1 { 0x88 } else { 0x89 }),
                    (Location::Reg(reg), src) =>
                        self.emit_r_rm(op_size, &src, reg as u8, if op_size == 1 { 0x8A } else { 0x8B }),
                    // Operands cannot both be memory.
                    _ => unreachable!(),
                }
            }
    ...
    fn emit_movabs(&mut self, dst: Operand, src: Operand) {
        ...
        self.emit_u8(if op_size == 1 { opcode } else { opcode + 1 });
        self.emit_imm(op_size, imm);
    }
    ...
    fn emit_movzx(&mut self, dst: Register, src: Location) {
        ...
        self.emit_r_rm(dst_size, &src, dst as u8, if src_size == 1 { 0x0FB6 } else { 0x0FB7 });
    }
    ...
    fn emit_setcc(&mut self, dst: Location, cc: ConditionCode) {
        assert!(dst.size() == 1);
        self.emit_r_rm(1, &dst, 0, 0x0F90 + cc as u8 as u32);
    }
            ...
            Operand::Imm(imm) => {
                ...
                if let Location::Reg(reg) = dst {
                    if reg as u8 & 0xF == 0 {
                        ...
                        self.emit_u8(if op_size == 1 { 0xA8 } else { 0xA9 });
                        self.emit_imm(if op_size == 8 { 4 } else { op_size }, imm);
                        return;
                    }
                }

                self.emit_r_rm(op_size, &dst, 0, if op_size == 1 { 0xF6 } else { 0xF7 });
                self.emit_imm(if op_size == 8 { 4 } else { op_size }, imm);
            }

            Operand::Reg(reg) => {
                assert_eq!(reg.size(), op_size);
                self.emit_r_rm(op_size, &dst, reg as u8, if op_size == 1 { 0x84 } else { 0x85 });
            }
    ...
    fn emit_xchg(&mut self, dst: Location, src: Location) {
        ...
        self.emit_r_rm(op_size, &dst, src as u8, if op_size == 1 { 0x86 } else { 0x87 });
    }
    ...
    pub fn encode(&mut self, op: Op) {
        match op {
            ...
            Op::Imul1(src) => {
                ...
                self.emit_r_rm(op_size, &src, 5, if op_size == 1 { 0xF6 } else { 0xF7 });
            }
            Op::Imul2(dst, src) => {
                let op_size = dst.size();
                assert!(op_size != 1);
