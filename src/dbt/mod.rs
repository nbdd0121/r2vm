...
extern "C" fn handle_trap(ctx: &mut crate::riscv::interp::Context, pc: usize) {
    let blk = unsafe { &*(ctx.cur_block as *const DbtBlock) };
    let i = blk.get_index_by_pc(pc - blk.code.as_slice().as_ptr() as usize);
...
pub struct DbtBlock {
    ...
    pub block: Box<[(Op, bool)]>,
    pub code: crate::util::Code,
    pub pc_map: Vec<u8>,
    pub pc_start: u64,
    pub pc_end: u64,
}
...
impl DbtBlock {
    ...
    pub fn get_index_by_pc(&self, mut host_pc_offset: usize) -> usize {
        for (i, &size) in self.pc_map.iter().enumerate() {
...
impl DbtCompiler {
    ...
    pub fn compile(&mut self, block: (Vec<(Op, bool)>, u64, u64)) -> DbtBlock {
        // Compilation requires Op to be 'static
        let opblock = block.0.into_boxed_slice();
